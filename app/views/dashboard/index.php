...
<div class="page-header">
    ...
    <a href="/finanzas/public/?c=transacciones&a=create" class="btn btn-primary">+ Nueva Transacción</a>
</div>
...
<div class="grid-2">
    ...
    <div class="card">
        <div class="card-header">
            ...
            <a href="/finanzas/public/?c=cuentas&a=index" class="btn btn-secondary btn-sm">Ver todas</a>
        </div>
        <?php if (empty($cuentas)): ?>
            <div class="empty-state"><div class="empty-icon">🏦</div><h3>Sin cuentas creadas</h3></div>
        <?php else: ?>
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;padding:.6rem 0;border-bottom:1px solid var(--border);">
                ...
                <div class="font-bold <?= $c['saldo'] >= 0 ? 'text-green' : 'text-red' ?>"><?= fmt((float)$c['saldo']) ?></div>
            </div>
            <?php endforeach; ?>
        <?php endif; ?>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            ...
            <a href="/finanzas/public/?c=recordatorios&a=index" class="btn btn-secondary btn-sm">Ver todos</a>
        </div>
        <?php if (empty($proximos)): ?>
            <div class="empty-state"><div class="empty-icon">🎉</div><h3>Sin vencimientos próximos</h3></div>
        <?php else: ?>
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;padding:.6rem 0;border-bottom:1px solid var(--border);">
                <div>
                    ...
                    <div class="text-sm">
                        <span class="badge <?= $r['tipo']==='pagar' ? 'badge-red' : 'badge-green' ?>"><?= $r['tipo'] ?></span>
                        &nbsp;<?= date('d/m/Y', strtotime($r['fecha_vencimiento'])) ?>
                    </div>
                </div>
                <div>
                    <div class="font-bold"><?= fmt((float)$r['monto']) ?></div>
                    <a href="/finanzas/public/?c=recordatorios&a=marcarPagado&id=<?= $r['id'] ?>" class="btn btn-sm btn-secondary">✓ Pagar</a>
                </div>
            </div>
            <?php endforeach; ?>
        <?php endif; ?>
...
<div class="card mt-4">
    <div class="card-header">
        ...
        <a href="/finanzas/public/?c=transacciones&a=index" class="btn btn-secondary btn-sm">Ver todas</a>
    </div>
    <?php if (empty($ultimas)): ?>
        <div class="empty-state">
            <div class="empty-icon">💸</div>
            <h3>Sin transacciones aún</h3>
            <a href="/finanzas/public/?c=transacciones&a=create" class="btn btn-primary mt-3">Crear primera transacción</a>
        </div>
    <?php else: ?>
    <div class="table-wrapper">
        <table>
            <thead><tr><th>Fecha</th><th>Descripción</th><th>Cuenta</th><th>Negocio</th><th>Tipo</th><th>Monto</th></tr></thead>
            <tbody>
            ...
            <tr>
                ...
                <td class="<?= $t['tipo']==='ingreso' ? 'amount-income' : ($t['tipo']==='gasto' ? 'amount-expense' : 'amount-transfer') ?>">
                    <?= fmt((float)$t['monto']) ?>
                </td>
            </tr>
            <?php endforeach; ?>
...
<div class="card mt-4">
    ...
    <div class="table-wrapper">
        <table>
            <thead><tr><th>Negocio</th><th>Tipo</th><th>Total</th></tr></thead>
            <tbody>
            <?php foreach ($porNegocio as $row): ?>
            <tr>
                <td class="td-bold"><?= htmlspecialchars($row['nombre']) ?></td>
                <td><span class="badge <?= $row['tipo']==='ingreso' ? 'badge-green' : ($row['tipo']==='gasto' ? 'badge-red' : 'badge-blue') ?>"><?= $row['tipo'] ?></span></td>
