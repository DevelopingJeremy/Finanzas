            ...
            <div class="card">
                    ...
                    <div class="form-group">
                        ...
                        <select id="categoria_id" name="categoria_id" class="form-control">
                            ...
                            <?php foreach ($categorias as $cat): ?>
                                <option value="<?= $cat['id'] ?>"><?= htmlspecialchars($cat['icono'] . ' ' . $cat['nombre']) ?>
                                    (<?= $cat['tipo'] ?>)</option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <select id="cuenta_id" name="cuenta_id" class="form-control" onchange="onCuentaChange(this)"
                            required>
                            <option value="">— Seleccionar —</option>
                            <?php foreach ($cuentas as $c): ?>
                                <option value="<?= $c['id'] ?>"><?= htmlspecialchars($c['nombre']) ?>
                                    (₡<?= number_format((float) $c['saldo'], 0, ',', '.') ?>)</option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="descripcion" name="descripcion" class="form-control" rows="3"
                        placeholder="Detalle de la transacción..."></textarea>
                </div>

                <button type="submit" class="btn btn-primary w-full" style="justify-content:center;">💾 Guardar
                    Transacción</button>
            </div>
        </div>
