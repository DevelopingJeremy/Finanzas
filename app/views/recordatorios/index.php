...
<div class="page-header">
    <div>
        <h1>🔔 Recordatorios</h1>
        <p>Pagos y cobros programados</p>
    </div>
    <a href="/public/?c=recordatorios&a=create" class="btn btn-primary">+ Nuevo</a>
</div>
...
<?php if (empty($recordatorios)): ?>
    <div class="card">
        <div class="empty-state">
            ...
            <h3>Sin recordatorios</h3><a href="/public/?c=recordatorios&a=create" class="btn btn-primary mt-3">Crear
                Recordatorio</a>
        </div>
    </div>
<?php else: ?>
                ...
                <tbody>
                    <?php foreach ($recordatorios as $r): ?>
                        ...
                        <tr>
                            ...
                            <td class="text-muted"><?= $frecuenciaLabel[$r['frecuencia'] ?? 'ninguna'] ?? $r['frecuencia'] ?>
                            </td>
                            <td>
                            ...
                            <td>
                                <div style="display:flex;gap:.4rem;flex-wrap:wrap;">
                                    <?php if ($r['estado'] === 'pendiente'): ?>
                                        <a href="/public/?c=recordatorios&a=marcarPagado&id=<?= $r['id'] ?>"
                                            class="btn btn-secondary btn-sm"
                                            onclick="return confirm('¿Confirmar pago/cobro? Se afectará el saldo de la cuenta.')">
                                            ✓ <?= $r['tipo'] === 'pagar' ? 'Pagar' : 'Cobrar' ?>
                                        </a>
                                    <?php endif; ?>
                                    <a href="/public/?c=recordatorios&a=edit&id=<?= $r['id'] ?>"
                                        class="btn btn-secondary btn-sm">✏️</a>
                                    <button
                                        onclick="confirmDelete('/public/?c=recordatorios&a=delete&id=<?= $r['id'] ?>','<?= htmlspecialchars($r['nombre']) ?>')"
