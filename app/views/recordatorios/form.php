<div class="page-header">
    <div>
        <h1>🔔 <?= $accion ?> Recordatorio</h1>
    </div>
    <a href="/public/?c=recordatorios&a=index" class="btn btn-secondary">← Volver</a>
</div>
...
<div class="card" style="max-width:680px;">
    ...
    <form method="POST">
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" id="nombre" name="nombre" class="form-control"
                    value="<?= htmlspecialchars($recordatorio['nombre'] ?? '') ?>"
                    placeholder="Ej: Alquiler, Cuota tarjeta" required>
            </div>
            <div class="form-group">
                ...
                <select id="tipo" name="tipo" class="form-control" required onchange="toggleCliente(this.value)">
                    ...
                    <option value="cobrar" <?= ($recordatorio['tipo'] ?? '') === 'cobrar' ? 'selected' : '' ?>>💰 Cobrar
                    </option>
                </select>
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select id="frecuencia" name="frecuencia" class="form-control">
                    <option value="ninguna" <?= ($recordatorio['frecuencia'] ?? 'ninguna') === 'ninguna' ? 'selected' : '' ?>>
                        Sin repetición</option>
                    <option value="diario" <?= ($recordatorio['frecuencia'] ?? '') === 'diario' ? 'selected' : '' ?>>Diario
                    </option>
                    <option value="semanal" <?= ($recordatorio['frecuencia'] ?? '') === 'semanal' ? 'selected' : '' ?>>Semanal
                    </option>
                    <option value="mensual" <?= ($recordatorio['frecuencia'] ?? '') === 'mensual' ? 'selected' : '' ?>>Mensual
                    </option>
                </select>
            </div>
            <div class="form-group">
                ...
                <select id="negocio_id" name="negocio_id" class="form-control">
                    ...
                    <?php foreach ($negocios as $n): ?>
                        <option value="<?= $n['id'] ?>" <?= ($recordatorio['negocio_id'] ?? '') == $n['id'] ? 'selected' : '' ?>>
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                <label class="form-label" for="cuenta_id">Cuenta <span class="text-muted" style="font-size:.8em;">(se
                        afectará al pagar)</span></label>
                <select id="cuenta_id" name="cuenta_id" class="form-control" onchange="filtrarBolsillos(this.value)">
                    <option value="">— Sin cuenta —</option>
                    <?php foreach ($cuentas as $c): ?>
                        <option value="<?= $c['id'] ?>" <?= ($recordatorio['cuenta_id'] ?? '') == $c['id'] ? 'selected' : '' ?>>
        ...
        <div class="form-row">
            <div class="form-group">
                <label class="form-label" for="subcuenta_id">Bolsillo / Subcuenta <span class="text-muted"
                        style="font-size:.8em;">(opcional, fijo)</span></label>
                <select id="subcuenta_id" name="subcuenta_id" class="form-control">
                    <option value="">— Sin bolsillo —</option>
                    <?php foreach ($subcuentas as $s): ?>
                        <option value="<?= $s['id'] ?>" data-cuenta="<?= $s['cuenta_id'] ?>"
            ...
            <div class="form-group" id="group_cliente"
                style="<?= ($recordatorio['tipo'] ?? '') === 'cobrar' ? '' : 'display:none;' ?>">
        ...
        <div style="display:flex;gap:.75rem;margin-top:1rem;">
            ...
            <a href="/public/?c=recordatorios&a=index" class="btn btn-secondary">Cancelar</a>
        </div>
    </form>
</div>
