...
<div class="page-header">
    <div>
        ...
        <p><a href="/public/?c=clientes&a=index" class="text-muted">← Clientes</a></p>
    </div>
</div>
...
<div class="grid-2">
    ...
    <div class="card">
        ...
        <form method="POST" action="/public/?c=clientes&a=crearPago&cliente_id=<?= $cliente['id'] ?>">
            ...
            <div class="form-group">
                ...
                <textarea id="descripcion" name="descripcion" class="form-control" rows="2"
                    placeholder="Concepto del pago..."></textarea>
            </div>
            <button type="submit" class="btn btn-primary w-full" style="justify-content:center;">💾 Registrar
                Pago</button>
        </form>
    </div>
    ...
    <div class="card">
        ...
        <?php else: ?>
            <?php foreach ($pagos as $p): ?>
                <div
                    style="display:flex;justify-content:space-between;align-items:center;padding:.65rem 0;border-bottom:1px solid var(--border);">
                    <div>
                        <div class="font-600" style="font-size:.875rem;"><?= fmtP((float) $p['monto']) ?></div>
                        <div class="text-sm text-muted"><?= date('d/m/Y', strtotime($p['fecha'])) ?> ·
                            <?= htmlspecialchars($p['descripcion'] ?: '—') ?></div>
                    </div>
                    <div style="display:flex;align-items:center;gap:.5rem;">
                        <span
                            class="badge <?= $p['estado'] === 'pagado' ? 'badge-green' : 'badge-yellow' ?>"><?= $p['estado'] ?></span>
                        <?php if ($p['estado'] === 'pendiente'): ?>
                            <a href="/public/?c=clientes&a=marcarPago&id=<?= $p['id'] ?>&cliente_id=<?= $cliente['id'] ?>"
