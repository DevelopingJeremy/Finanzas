<div class="page-header">
    <div><h1>👤 <?= $accion ?> Cliente</h1></div>
    <a href="/finanzas/public/?c=clientes&a=index" class="btn btn-secondary">← Volver</a>
</div>
...
<div class="card" style="max-width:600px;">
    ...
    <form method="POST">
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <select id="negocio_id" name="negocio_id" class="form-control">
                    ...
                    <option value="<?= $n['id'] ?>" <?= ($cliente['negocio_id']??'') == $n['id'] ? 'selected':'' ?>><?= htmlspecialchars($n['nombre']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea id="notas" name="notas" class="form-control" placeholder="Observaciones del cliente..."><?= htmlspecialchars($cliente['notas'] ?? '') ?></textarea>
        </div>
        <div style="display:flex;gap:.75rem;margin-top:1rem;">
            <button type="submit" class="btn btn-primary"><?= $accion ?> Cliente</button>
            <a href="/finanzas/public/?c=clientes&a=index" class="btn btn-secondary">Cancelar</a>
        </div>
    </form>
</div>
